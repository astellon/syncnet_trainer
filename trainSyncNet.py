...
parser.add_argument('--nOut', type=int,  default=1024,
...
parser.add_argument('--alphaI', type=float,
                    default=1.0, help='Identity weight')

## Load and save
parser.add_argument('--initial_model', type=str,
                    default="", help='Initial model weights')
parser.add_argument('--save_path',     type=str,
                    default="./data/exp01", help='Path for model and logs')

## Training and test data
parser.add_argument('--train_list', type=str, default="data/dev.txt", help='')
parser.add_argument('--verify_list', type=str,
                    default="data/test.txt", help='')

## Speaker recognition test
parser.add_argument('--test_list', type=str,
                    default="voxceleb/test_list.txt", help='Evaluation list')
parser.add_argument('--test_path', type=str,
                    default="voxceleb/voxceleb1", help='Absolute path to the test set')
...
if args.eval == True:
    ...
    sc, lab = s.evaluateFromListSave(
        args.test_list, print_interval=100, test_path=args.test_path)
...
trainLoader = DatasetLoader(
    args.train_list,  nPerEpoch=args.nTrainPerEpoch, **vars(args))
valLoader = DatasetLoader(
    args.verify_list, nPerEpoch=args.nTestPerEpoch, evalmode=True, **vars(args))
...
while(1):
    ...
    loss, trainacc = s.train_network(
        trainLoader, evalmode=False, alpI=args.alphaI, alpC=args.alphaC)
