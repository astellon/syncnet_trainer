...
class SyncNet(nn.Module):
    ...
    def __init__(self, model=None, maxFrames=200, lr=0.0001, nOut=1024, temporal_stride=1, **kwargs):
        ...
        SyncNetModel = importlib.import_module(
            model).__getattribute__("SyncNetModel")
    ...
    def train_network(self, loader=None, evalmode=None, alpC=1.0, alpI=1.0):
        ...
        for data in loader:
            ...
            idoutput = F.cosine_similarity(
                out_VA.expand(-1, -1, stepsize), out_AA.expand(-1, -1, stepsize).transpose(0, 2)) * self.__L__.wI + self.__L__.bI

            nloss_id = criterion(idoutput, label_id)

            p1i, p5i = accuracy(idoutput.detach().cpu(),
                                label_id.detach().cpu(), topk=(1, 2))
    ...
    def evaluateFromListSave(self, listfilename, print_interval=10, test_path='', num_eval=10):
        ...
        for idx, file in enumerate(setfiles):
            ...
            inp1 = loadWAV(os.path.join(
                test_path, file), self.__max_frames__*4, evalmode=True, num_eval=num_eval).cuda()
